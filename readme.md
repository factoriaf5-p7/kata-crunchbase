...
- From a terminal pointing to the folder project, run the command `nest new` ---**IMPORTANT Do not write project name**
- When prompted the project name, type: . (a dot). It will build the project inside the current folder.
- Use the `companies.json` file as a database.
- Build the endpoints to answer the following queries:

  1. List by name all companies.
  2. Filter companies by foundation year.
  3. Filter companies by city office
  4. Filter companies by tag.
  5. Company details by Id or by name.

<detail>
<summary>Check the one document structure to guide your query implementation</summary>
...
Build a React Interface to connect to the API.

## Bonus Track II

Migrate data to a mongo database.
